# Autogenerated wrapper script for MPICH_jll for x86_64-apple-darwin14-libgfortran3
...
const libmpifort_splitpath = ["lib", "libmpifort.12.dylib"]
...
const libmpifort = "@rpath/libmpifort.12.dylib"


# Relative path to `libmpicxx`
const libmpicxx_splitpath = ["lib", "libmpicxx.12.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmpicxx = "@rpath/libmpicxx.12.dylib"


# Relative path to `libmpi`
const libmpi_splitpath = ["lib", "libmpi.12.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmpi = "@rpath/libmpi.12.dylib"
...
function mpiexec(f::Function; adjust_PATH::Bool = true, adjust_LIBPATH::Bool = true)
    ...
    if adjust_LIBPATH
        if !isempty(get(ENV, LIBPATH_env, expanduser("~/lib:/usr/local/lib:/lib:/usr/lib")))
            env_mapping[LIBPATH_env] = string(LIBPATH, ':', get(ENV, LIBPATH_env, expanduser("~/lib:/usr/local/lib:/lib:/usr/lib")))
        else
